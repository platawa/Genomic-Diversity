#!/usr/bin/env python3
"""
identify_umap_clusters.py

Given a chr12 UMAP plot with visually identified clusters, extract the
genomic regions in each cluster and cross-reference with GTF annotations.

Works by loading cluster_assignments.tsv (which has umap_1, umap_2 coords)
...
Usage:
    ...
    python tools/identify_umap_clusters.py \
        --latent_dir results/chr12/sae/latent_analysis_postnorm \
        --mode explore

    # Define visual clusters by UMAP coordinate bounding boxes and
...
def mode_explore(ca, embedding):
    ...
    # Check if umap coords are in the TSV or need numpy
    has_umap_cols = "umap_1" in ca.columns and "umap_2" in ca.columns
    if has_umap_cols:
        print("UMAP coordinates found in cluster_assignments.tsv")
    elif embedding is not None:
        print(f"UMAP coordinates from embedding_umap.npy: shape {embedding.shape}")
        ca = ca.copy()
        ca["umap_1"] = embedding[:, 0]
        ca["umap_2"] = embedding[:, 1]
    else:
    ...
    if "cluster" in ca.columns:
        ...
        for cid, count in cluster_counts.items():
            ...
            if "umap_1" in subset.columns:
                x_range = f"  UMAP x: [{subset['umap_1'].min():.1f}, {subset['umap_1'].max():.1f}]"
                y_range = f"  y: [{subset['umap_2'].min():.1f}, {subset['umap_2'].max():.1f}]"
            else:
                x_range = y_range = ""
...
def mode_annotate(ca, embedding, intervals, gene_names, output_dir, repeats=None, ccres=None):
    ...
    # Ensure UMAP coords are available
    if "umap_1" not in ca.columns and embedding is not None:
        ca = ca.copy()
        ca["umap_1"] = embedding[:, 0]
        ca["umap_2"] = embedding[:, 1]
    ...
    if "cluster" in ca.columns:
        ...
        for cid in sorted(ca["cluster"].unique()):
            ...
            if "umap_1" in subset.columns:
                print(f"  UMAP center: ({subset['umap_1'].mean():.1f}, {subset['umap_2'].mean():.1f})")
                print(f"  UMAP range:  x=[{subset['umap_1'].min():.1f}, {subset['umap_1'].max():.1f}]  "
                      f"y=[{subset['umap_2'].min():.1f}, {subset['umap_2'].max():.1f}]")
...
def mode_plot(ca, embedding, intervals, gene_names, output_dir, chrom, repeats=None, ccres=None):
    """Generate annotated UMAP plot with cluster labels, top genes, and annotation breakdown."""
    # Ensure UMAP coords
    if "umap_1" not in ca.columns and embedding is not None:
        ca = ca.copy()
        ca["umap_1"] = embedding[:, 0]
    ...
    # --- Figure 1: Overview with all clusters labeled ---
    fig, ax = plt.subplots(1, 1, figsize=(18, 16))

    # Plot all points colored by annotation
    for ann in ann_order:
        mask = ca["annotation"] == ann
        count = mask.sum()
        if count == 0:
            continue
        ax.scatter(ca.loc[mask, "umap_1"], ca.loc[mask, "umap_2"],
    ...
    if "cluster" in ca.columns:
        ...
        for cid in cluster_ids:
            ...
            cx = subset["umap_1"].mean()
            cy = subset["umap_2"].mean()
            sx = subset["umap_1"].std() * 2.0
            sy = subset["umap_2"].std() * 2.0
            ...
            lw = 2.0 if top_ann_pct >= 50 else 1.0  # thicker if strongly dominant
            ...
            # Only label clusters with >= 100 regions to reduce clutter
            if total >= 100:
    ...
    ax.set_title(f"UMAP of SAE Region Fingerprints — {chrom} (N={len(ca)})\n"
                 f"Colored by Genomic Annotation", fontsize=13)
    ax.legend(loc="upper right", fontsize=10, markerscale=4,
              framealpha=0.9, edgecolor="black")

    out_path = os.path.join(output_dir, f"umap_annotated_clusters_{chrom}.png")
    ...
    if "cluster" in ca.columns and len(cluster_ids) > 0:
        ...
        for idx, cid in enumerate(top_clusters):
            ...
            ax2.scatter(ca["umap_1"], ca["umap_2"], c="#e0e0e0", s=1, alpha=0.3, rasterized=True)

            # Highlight this cluster colored by annotation
            for ann in ann_order:
                mask = (ca["cluster"] == cid) & (ca["annotation"] == ann)
                if mask.sum() > 0:
                    ax2.scatter(ca.loc[mask, "umap_1"], ca.loc[mask, "umap_2"],
        ...
        fig2.tight_layout()
        out_path2 = os.path.join(output_dir, f"umap_cluster_details_{chrom}.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Identify UMAP clusters and cross-reference with GTF annotations")
    ...
    parser.add_argument("--mode", choices=["explore", "annotate", "export", "plot"],
                        ...
                        help="Mode: explore (inspect data), annotate (classify & summarize), "
                             "export (per-cluster TSV files), plot (annotated UMAP figure)")
    ...
    emb_path = os.path.join(data_dir, "embedding_umap.npy")
