...
This script:
  1. Finds shard directories for the chromosome (COMPLETED or with chunk data)
  2. Merges sae_results.tsv, signature_features.tsv, and feature_matrices.npz
  3. Computes normalization stats from chunk files (streaming, low memory)
  4. Re-runs signature-feature finding across merged regions
  5. Writes a new unified sae directory and marks it COMPLETED

Works with shards that OOM'd during step 6b — only needs _chunk_*.npz files
and _checkpoint_meta.json from each shard.
...
Usage:
    ...
    python merge_sae_shards.py --chrom chr1 --output_dir results/ --include-partial
"""
...
def merge_feature_matrices(shard_dirs: list[tuple], out_path: str) -> dict:
    ...
    into memory at once.  Each array is written as a compressed .npy inside
    the zip (the standard npz format).
...
def main():
    ...
    write_completed(run_dir, script_name='merge_sae_shards.py', wall_time_s=_wall_time)
